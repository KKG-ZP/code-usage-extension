...
代码用量监控是一个 GNOME Shell 扩展，用于从本地编码代理日志中统计 API 使用情况，包括 Token 消耗、请求数、缓存命中率和费用估算。支持 Claude Code、Codex、Gemini、Kimi、Qwen、GitHub Copilot CLI、OpenCode、Goose、Hermes、Kilo、ZCode 等代理源。

## 系统要求

- GNOME Shell 45-49
- `glib-compile-schemas`（通常随 GNOME/GLib 安装）
- 可选：`sqlite3`，仅 OpenCode、Goose、Hermes、Kilo、ZCode 等 SQLite 数据源需要
...
如果需要读取 OpenCode、Goose、Hermes、Kilo 或 ZCode 的 SQLite 数据，请安装 `sqlite3`：
...
- ZCode 数值前出现 `≈`：表示 ZCode 数据库记录为 0，扩展正在使用本地 rollout 请求负载估算 Token。
